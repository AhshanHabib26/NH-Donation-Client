  ...
  return (
    <Container>
      <div className="text-center mb-8">
        <p className="text-lg font-light text-[#D53F34]">Our Regular Program</p>
              ...
              {gallaryData.map((gallery) => (
                <SwiperSlide key={gallery.id}>
                  <div className="p-3 max-w-5xl mx-5 lg:mx-auto shadow-lg border border-dashed border-gray-100  rounded-lg">
                    <img className="w-full h-[300px] lg:h-[400px] object-cover  rounded-t-lg" src={gallery.image} alt="" />
                    <h1 className=" mb-6 p-2 text-center text-lg font-medium text-white rounded-b-lg" style={{ backgroundColor: getRandomColor() }}>{gallery.title}</h1>
                  </div>
                </SwiperSlide>
              ))}
            </div>
